...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.setInterval(function () {
            for (let t = 0; t < Math.floor(2499/3); t++) {
                main()
            }
        }, 1)
    ...
    class Network {
        constructor(inputs, layerSetupArray) {
            ...
            this.momentum = 0.001551565
            this.learningRate = 0.001551565
            ...
            for (let t = 0; t < this.nodeCount; t++) {
                let cluster = []
                for (let k = 0; k < Math.floor(Math.random() * this.nodeCount * 30) + 30; k++) {
    ...
    let inputs = []
    for (let t = 0; t < 44; t++) {
        inputs.push(new Data(0))
    }
    let net = new Network(inputs, [97, 3])
    let count = 0
    let pix = canvas_context.getImageData(0, 0, 50, 50)
    let worlderror = 0
    let x = 0
    let y = 0
    let cgo = 0
    ...
    arr.sort((a, b) => Math.random() > .5 ? 1 : -1)
        ...
        if (keysPressed['n']) {
            net = new Network(inputs, [33, 33, 33, 3])
            return
        }
        canvas_context.clearRect(0, 0, canvas.width * .5, canvas.height)  // refreshes the image
        gamepadAPI.update() //checks for button presses/stick movement on the connected controller)
        // game code goes here
        canvas_context.drawImage(pom, 0, 0, pom.width, pom.width, 0, 0, 50, 50)
        // return
        if (keysPressed['e']) {
            count += 1000
        }
        if (cgo < 1000 || keysPressed[' ']) {
            pix = canvas_context.getImageData(0, 0, 50, 50)
            cgo++
        }
        if (Math.random() < .01) {
            cgo = 0
        ...
        c++
        x = arr[c % 2500][0]//Math.floor(x*1)
        y = arr[c % 2500][1]// Math.floor(y*1)
        let kinputs = []
        for (let l = .1; l < 1.1; l += .1) {
            kinputs.push(new Data((Math.cos((((x - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
            kinputs.push(new Data((Math.sin((((y - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
            kinputs.push(new Data((Math.sin((((x - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
            kinputs.push(new Data((Math.cos((((y - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
        } 
        net.zompute(kinputs)

        canvas_context.fillStyle = `rgb(${net.outputs[net.outputs.length - 3] * 255}, ${net.outputs[net.outputs.length - 2] * 255}, ${net.outputs[net.outputs.length - 1] * 255})`
        canvas_context.fillRect(50 + x, y, 1, 1)


        // x = arr[t][0]//Math.floor(x*1)
        // y  = arr[t][1]// Math.floor(y*1)
        let index = Math.floor((x + (y * 50)) * 4)
        r = (pix.data[index] / 255)*(pix.data[index+3]/255)//x/50 //(y>25?1:0) // ((Math.cos(x/3)*.5)+.5)//pix.data[index]/255//
        g = (pix.data[index + 1] / 255)*(pix.data[index+3]/255)//(x>25?1:0) // ((Math.cos(x/3)*.5)+.5)//pix.data[index+1]/255
        b = (pix.data[index + 2] / 255)*(pix.data[index+3]/255)//(x>25?1:0) // ((Math.cos(x/3)*.5)+.5)//pix.data[index+2]/255
        if (keysPressed['h']) {
            canvas_context.fillStyle = `rgb(${net.outputs[net.outputs.length - 3] * 255}, ${net.outputs[net.outputs.length - 2] * 255}, ${net.outputs[net.outputs.length - 1] * 255})`
            canvas_context.fillRect(0, 0, canvas.width * .5, canvas.height)
        } else {
            let inputs = []
            for (let t = 0; t < net.ztructure[0].length - 3; t++) {
                // if (Math.random() > .05) {
                    inputs.push(new Data(net.ztructure[0][t].valueOf()))
                // } else {
            ...
            //    inputs.push(new Data(((Math.sign(r-net.outputs[net.outputs.length-3])*.5)+.5)*2))
            //    inputs.push(new Data(((Math.sign(g-net.outputs[net.outputs.length-2])*.5)+.5)*2))
            //    inputs.push(new Data(((Math.sign(b-net.outputs[net.outputs.length-1])*.5)+.5)*2))


            inputs.push(new Data(r))
            inputs.push(new Data(g))
            inputs.push(new Data(b))

            if (!keysPressed['q']) {
                net.zalculateDeltasSigmoid(inputs)
                net.zadjustWeights()
            }
            canvas_context.fillStyle = `rgb(${net.outputs[net.outputs.length - 3] * 255}, ${net.outputs[net.outputs.length - 2] * 255}, ${net.outputs[net.outputs.length - 1] * 255})`
            count++
            if (count > 25000) {
                ...
                // for (let t = 0; t < arr.length; t++) {
                    ...
                    let kinputs = []
                    for (let l = .1; l < 1.1; l += .1) {
                        kinputs.push(new Data((Math.cos((((xz - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
                        kinputs.push(new Data((Math.sin((((yx - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
                        kinputs.push(new Data((Math.sin((((xz - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
                        kinputs.push(new Data((Math.cos((((yx - 0) * ((l / 1) + 0)) / 50) * 1 * 2 * Math.PI) * .5) +.5))
                    }
                    net.zompute(kinputs)
                    canvas_context.fillStyle = `rgb(${net.outputs[net.outputs.length - 3] * 255}, ${net.outputs[net.outputs.length - 2] * 255}, ${net.outputs[net.outputs.length - 1] * 255})`
                    let index = Math.floor((xz + (yx * 50)) * 4)
                    r = pix.data[index] / 255
                    g = pix.data[index + 1] / 255
                    b = pix.data[index + 2] / 255
                    worlderror += Math.abs(net.outputs[net.outputs.length - 3] - r)
                    worlderror += Math.abs(net.outputs[net.outputs.length - 2] - g)
                    worlderror += Math.abs(net.outputs[net.outputs.length - 1] - b)
                    canvas_context.fillRect(50 + xz, yx, 1, 1)
                }
            
                console.log(worlderror)
            }
...
// let g = {"ChatID":"3,6,2","Dialogue":[{"ID":3,"Says":"Bad morning"}, {"ID":6,"Says":"Bad morning to you as well"}, {"ID":3,"Says":"Fuck off!"}], "DependsOn":["12,7,2", "3,6,1"]}